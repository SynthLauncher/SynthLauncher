...
use crate::utils::{self, download::DownloadErr, errors::BackendError, zip::ZipExtractor};
use bytes::Bytes;
use futures::{stream::FuturesUnordered, StreamExt};
use sha1::{Digest, Sha1};
use synthlauncher_meta::json::{
    asset_index::{AssetIndex, AssetObject},
    client::{Client, Download, Library},
};
...
#[inline(always)]
async fn download_and_verify(download: &Download, path: &Path) -> Result<(), DownloadErr> {
...
async fn download_and_read_file(download: &Download, path: &Path) -> Result<Bytes, DownloadErr> {
...
async fn download_to(download: &Download, path: &Path) -> Result<(), DownloadErr> {
    let full_path = if let Some(ref child) = download.path {
        &path.join(child)
    } else {
        path
...
async fn install_assets(assets_root: &Path, client: &Client) -> Result<(), DownloadErr> {
    ...
    let download_object = async |object: AssetObject| -> Result<(), DownloadErr> {
