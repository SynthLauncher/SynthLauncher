...
mod tests {
    ...
    fn deserialize_os_name() {
        let json = "\"windows\"";
        let os_name: OsName = from_str(json).unwrap();
    ...
    fn deserialize_arch() {
        let json = "\"x86\"";
        let arch: Arch = from_str(json).unwrap();
